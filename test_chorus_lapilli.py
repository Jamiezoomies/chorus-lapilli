...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    # Additional Tests
    def test_button_click_after_a_player_wins(self):
        '''Check if clicking is disabled after a player has won.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
        ...
        # arbitary clicks after a winner determined
        tiles[0].click()
        tiles[8].click()
        tiles[7].click()
        tiles[5].click()
    ...
    def test_unacceptable_select_and_move(self):
        '''Check if clicking to move a piece to unacceptable (non-adjacent or occupied) square is not allowed.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
        self.assertTileIs(tiles[5], self.SYMBOL_BLANK)
    ...
    def test_must_vacate_center_if_you_cant_win(self):
        '''Check if, when the player occupies the center square, it is acceptable to move a non-center piece, provided that the player can win in this move.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
        self.assertTileIs(tiles[5], self.SYMBOL_BLANK)
    ...
    def test_can_win_without_vacating_center(self):
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
        self.assertTileIs(tiles[1], self.SYMBOL_BLANK)
